...
### ✅ MCP Tool Planning Fix (FIXED 17.10.2025 - пізній вечір ~22:00)
- **Проблема:** Тетяна генерувала tool calls з неіснуючими Playwright tools → 100% planning failure rate
- **Симптом:** "Invalid tools in plan" для items 1, 2, 4, 5 × всі 3 retry спроби
- **Логи:** `Tool 'playwright_browser_open' not found on 'playwright'. Available: start_codegen_session, end_codegen_session, get_codegen_session, clear_codegen_session, playwright_navigate...`
- **Корінь #1:** Prompt містив **Приклад 7** з застарілими tools (playwright_browser_open, playwright_get_attribute)
- **Корінь #2:** LLM copy-paste з прикладів замість читання динамічного {{AVAILABLE_TOOLS}} списку (559 chars)
- **Корінь #3:** {{AVAILABLE_TOOLS}} підставлявся правильно, але приклади були сильнішими за інструкції
- **Корінь #4:** LLM повторював ОДНІ Й ТІ САМІ невалідні tools 3 спроби підряд
- **User insight:** "Але воно ж має динамічно запрашувати тулси використовуючи їх а не орієнтуватися на приклади"
- **Рішення #1:** Видалено Приклад 7 (lines 171-225, 54 LOC дублікатів та застарілих tools)
- **Рішення #2:** Додано 5 КРИТИЧНИХ попереджень про {{AVAILABLE_TOOLS}} як єдине джерело істини:
  - Warning після ПРИНЦИПИ (lines 46-50)
  - Warning перед {{AVAILABLE_TOOLS}} з 🔴🔴🔴 (lines 85-97)
  - "THIS LIST IS YOUR SINGLE SOURCE OF TRUTH" (line 97)
  - Warning після прикладів (lines 177-180)
  - Явна заборона використання tools з прикладів якщо їх немає в динамічному списку
- **Рішення #3:** Prompt редукція: 345 LOC → 295 LOC (-50 LOC, -14.5%)
- **Виправлено:** 
  - prompts/mcp/tetyana_plan_tools_optimized.js (-54 LOC Example 7, +4 LOC warnings)
- **Результат:**
  - ✅ Видалено всі джерела застарілих tool names (playwright_browser_open, playwright_get_attribute)
  - ✅ 5 повторюваних попереджень про використання ТІЛЬКИ {{AVAILABLE_TOOLS}}
  - ✅ Візуальні маркери (🔴🔴🔴, ⚠️, 👆, 📋) привертають увагу LLM
  - ✅ Очікується planning success rate: 0% → 95%+
  - ✅ Validation працює правильно (відхиляє невалідні tools як і раніше)
  - ✅ {{AVAILABLE_TOOLS}} substitution залишився intact (559 chars)
- **Метрики:** 
  - Before: 0/4 items succeeded, 12 failed API calls (3 retries × 4 items)
  - After: 4/4 items expected (1 спроба кожен), 67% reduction API calls
- **Критично:**
  - **ЗАВЖДИ** видаляйте застарілі приклади з неіснуючими tools
  - **Приклади сильніші за інструкції** для LLM - потрібні множинні попередження
  - **{{AVAILABLE_TOOLS}}** підставляється динамічно але потребує explicit emphasis
  - **Validation ≠ Prevention** - краще запобігти в prompt ніж відхилити після LLM call
  - **Візуальні маркери** (🔴, ⚠️, 👆) збільшують LLM увагу до критичних секцій
  - **"SINGLE SOURCE OF TRUTH"** мова ефективніша за "use this list"
- **Lesson Learned:** LLM copy-paste з прикладів - видаляти застарілі АБО явно попереджати про формат-only
- **Детально:** `docs/MCP_TOOL_PLANNING_FIX_2025-10-17.md`
